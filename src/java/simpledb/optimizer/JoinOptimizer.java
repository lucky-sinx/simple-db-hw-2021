...
public class JoinOptimizer {
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        Set<Set<LogicalJoinNode>> subsets=new HashSet<>();
        Set<LogicalJoinNode> finalSet=new HashSet<>();
        PlanCache planCache = new PlanCache();
        for (int i = 1; i <= this.joins.size(); i++) {
            //对于不同长度的子集合进行处理，找到该长度最优的
            subsets = enumerateSubsets(this.joins, i);//获取该长度子集合 subsets={subset1(length=i),subset2,...}
            for (Set<LogicalJoinNode> subset : subsets) {//遍历subsets
                if(i==this.joins.size()){
                    finalSet=subset;
                }
                double optCosts = Double.MAX_VALUE;
                CostCard bestPlan = null;
                for (LogicalJoinNode logicalJoinNode : subset) {//对于subset中的每一个join节点计算cost
                    CostCard cost_a = computeCostAndCardOfSubplan(
                            stats,
                            filterSelectivities,
                            logicalJoinNode,
                            subset,
                    ...
                    if(cost_a!=null){
                        //不是null就说明已经找到了bestCostSoFar,更新optCost、bestPlan
                        optCosts=cost_a.cost;
                        bestPlan=cost_a;
                    }
                }
                if(bestPlan!=null){
        ...
        return planCache.getOrder(finalSet);
        //return null;
    }
